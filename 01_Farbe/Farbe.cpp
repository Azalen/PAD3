...
void Farbe::ausgabe(std::ostream & ostr, bool withEnter) const {
    ostr << "R:" << setw(3) << r << "  G:" << setw(3) << g << "  B:" << setw(3) << b << "  A:" << setw(3) << a; 
    if(withEnter)
        ostr << endl;
}

Farbe Farbe::darker(const Farbe & toDarkenFrom) {
    return Farbe(toDarkenFrom.getR()*DEF_GREY_FAC, toDarkenFrom.getG()*DEF_GREY_FAC, toDarkenFrom.getB()*DEF_GREY_FAC, toDarkenFrom.getA());
}
